...
{
    internal class classPosPrintCaffePredracun
    {
        ...
        private static string _brojStola = "";
        ...
        public static bool predracun = false;
        public static bool ima_stavke_za_ispis = false;
        private static string napomena = "";
        public static string nap { get; set; }
        //a
        public static void PrintReceipt(DataTable DTstavke, string blagajnik, string broj_racuna, string kupac, string barcode, string brRac, string placanje, string stol)
        {
            DTpdv = new DataTable();
            napomena = "";
            BrojRacunaa = brRac;
            ...
            if (DTstavke.Columns.Contains("napomena"))
            {
                if (DTstavke.Rows[0]["napomena"].ToString().Trim().Length > 0)
                {
                    napomena = DTstavke.Rows[0]["napomena"].ToString();
                }
            }
            ...
            try
            {
                ...
                //PrintReceiptHeader(DT.Rows[0]["skraceno_ime"].ToString(), DT.Rows[0]["adresa"].ToString(), "My State, My Country", DT.Rows[0]["tel"].ToString(), DateTime.Now, blagajnik, brRac, kupac);
                PrintReceiptHeader(DT.Rows[0]["ime_tvrtke"].ToString(), DT.Rows[0]["skraceno_ime"].ToString(), DT.Rows[0]["adresa"].ToString(), "My State, My Country", DT.Rows[0]["tel"].ToString(), DateTime.Now, blagajnik, brRac, kupac);
        ...
        private static void StopePDVa(double pdv, double pdv_stavka, double osnovica)
        {
            ...
            {
                dataROW[0]["iznos"] = Convert.ToDouble(dataROW[0]["iznos"].ToString()) + pdv_stavka;
                dataROW[0]["osnovica"] = osnovica;
            }
        }
        ...
        private static void PrintReceiptHeader(string imeTvrtke, string companyName, string addressLine1, string addressLine2, string taxNumber, DateTime dateTime, string cashierName, string broj, string kupac)
        {
            if (predracun)
            {
                ...
                //PrintTextLine((string.Format("Predračun").ToUpper()));

                DataTable DTtable = classSQL.select("SELECT naziv FROM stolovi WHERE id_stol='" + _brojStola + "'", "stolovi").Tables[0];
                if (DTtable.Rows.Count > 0)
                {
                    PrintTextLine("Naziv stola: " + DTtable.Rows[0]["naziv"].ToString());
                }
                else
                {
                    PrintTextLine((string.Format("Predračun").ToUpper()));
